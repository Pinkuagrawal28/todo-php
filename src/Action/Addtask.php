...
<tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
  ...
  <td class="px-6 py-4">
    <a hx-get="/task?action=showedit&id=' . urlencode($lastId) . '"
  ...
  <td class="px-6 py-4">
    <a hx-get="/task?action=delete&id=' . urlencode($lastId) . '"
       class="text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">
       Delete
    </a>
  </td>
  <td class="px-6 py-4">
    <span id="mark-button-' . htmlspecialchars($lastId) . '"
          hx-get="/task?action=mark&id=' . urlencode($lastId) . '"
          hx-swap="outerHTML">
